@echo off
REM task22: 96 b = m2b4 WKV (48 b, per-head single norm) + m4b4 LEARNABLE-cb PQ shifts (2x(16 idx +
REM 8 norm) = 48 b), 2.0 ep. WAITS for the q128L GPU EVAL (serial queue; polls its log, max ~8 h).
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,960) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\gpu_eval_q128L.log" >nul 2>&1 && goto :run
...
echo ===== task22 96-b QAT (m2b4 + m4b4 learnable shift cb) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

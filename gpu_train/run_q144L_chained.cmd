@echo off
REM task22: 144 b with LEARNABLE shift codebook (RWKV_QAT_SHIFT_PQ_LEARN=1), 2.0 ep. WAITS for the q144
REM control training (polls its log for DONE_EXIT, max ~8 h). OMP=4 per the 14-thread budget.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,960) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\qat_qat_pq_q144.log" >nul 2>&1 && goto :run
  timeout /t 30 /nobreak >nul
)
:run
set CFG=qat_pq_q144L
...
echo ===== task22 144-b QAT LEARNABLE shift cb 2.0ep START %DATE% %TIME% ===== > "%LOG%"

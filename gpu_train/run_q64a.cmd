@echo off
REM task22: q64a = the q64r 64-b rotation config + SOFT-TO-HARD SELECTION ANNEALING (tau0 0.05,
REM fully hard from 50% of training - no end gap). Clean A/B vs q64rv +0.002492/+0.0012. SELF-QUEUED:
REM waits for the q64k verdict chain (strict serial queue), then trains. OMP 4 + fetch 4.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,1200) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\pq_q64k.log" >nul 2>&1 && goto :run
...
echo ===== task22 64-b ANNEAL A/B (tau0 0.05, hard at 50%%) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

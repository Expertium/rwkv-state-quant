@echo off
REM task22: 144 b = m2b4 WKV + PQ SHIFTS (fixed codebook control), 2.0 ep. WAITS for the q192 training
REM to free the GPU (polls its log for DONE_EXIT, max ~8 h). OMP=4 per the 14-thread budget.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,960) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\qat_qat_pq_q192.log" >nul 2>&1 && goto :run
...
echo ===== task22 144-b QAT (m2b4 + PQ shifts, fixed cb) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

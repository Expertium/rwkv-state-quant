@echo off
REM task22: 224-b retry at 2.0 ep. WAITS for the q256 training to free the GPU (polls its log for
REM DONE_EXIT, max ~8 h). Dedicated cmd — never edit while running.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,960) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\qat_qat_pq_q256.log" >nul 2>&1 && goto :run
  timeout /t 30 /nobreak >nul
)
:run
set CFG=qat_pq_q224e2
...
echo ===== task22 224-b QAT RETRY (m2b8 + shift-int2) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

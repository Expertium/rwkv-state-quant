@echo off
REM task22: q56k = the 64-b config + the two NEW QAT levers: KD from the fp32 champion (lambda 0.5)
...
for /L %%i in (1,1,1200) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\pq_q56k.log" >nul 2>&1 && goto :run
...
echo ===== task22 64-b RETRY with KD 0.2 + RESURRECTION 2.0ep START %DATE% %TIME% ===== > "%LOG%"

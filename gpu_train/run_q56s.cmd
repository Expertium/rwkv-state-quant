@echo off
REM task22: q56s = the 56-b KITCHEN SINK: m1b5 WKV + m4b4 shifts + rotation + ANNEAL (tau0 0.05,
REM hard at 50%) + KD 0.2 + resurrection. 56 b failed at +0.0032 (q56r) and +0.0034 (q56k). SELF-QUEUED:
REM waits for the q64a verdict chain (strict serial queue), then trains. OMP 4 + fetch 4.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,2400) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\pq_q64a.log" >nul 2>&1 && goto :run
...
echo ===== task22 56-b KITCHEN SINK (rot+anneal+KD0.2+resurrect) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

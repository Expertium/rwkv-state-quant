@echo off
REM task22 ladder: 192 b = m2b4 PQ + int2 shifts, 2.0 ep. WAITS for the q224e2 training to free the GPU
REM (polls its log for DONE_EXIT, max ~8 h). Dedicated cmd — never edit while running.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,960) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\qat_qat_pq_q224e2.log" >nul 2>&1 && goto :run
...
echo ===== task22 192-b QAT (m2b4 + shift-int2) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

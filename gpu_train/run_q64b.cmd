@echo off
REM task22: q64b = SEED-VARIANCE rerun of the q64r 64-b rotation config — IDENTICAL except
REM RWKV_AUGMENT_SEED 1234 -> 4321 (different batch order). Tests whether the razor-thin q64rv
REM win (+0.002492 imm, margin 0.000008) reproduces. SELF-QUEUED: waits for the q56s verdict
REM chain to finish first (strict serial queue), then trains. OMP 4 + fetch 4.
cd /d C:\Users\Andrew\rwkv-state-quant\gpu_train
for /L %%i in (1,1,1200) do (
  findstr /C:"DONE_EXIT" "C:\Users\Andrew\rwkv-state-quant\scratchpad\pq_q56s.log" >nul 2>&1 && goto :run
...
echo ===== task22 64-b SEED-VARIANCE rerun (q64r config, seed 4321) 2.0ep START %DATE% %TIME% ===== > "%LOG%"

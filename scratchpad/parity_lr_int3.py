...
of = quant_aware_rwkv7(*ins, skip, float("inf"), 1, QMAX)   # lowrank rank=1 int3 -> fused
gf = torch.autograd.grad(of, ins, torch.ones_like(of))

ins2=[t.detach().clone().requires_grad_(True) for t in base]
oref = ref_qat_deploy_rank1(*ins2, skip, QMAX)
gref = torch.autograd.grad(oref, ins2, torch.ones_like(oref))

od=(of-oref).abs()
den=oref.abs().amax().clamp_min(1e-6)
print(f"[int3 fqmax={QMAX}] FWD fused vs deploy-rank1 ref: max {od.max():.3e} mean {od.mean():.3e}  (max/scale {od.max()/den:.2e})")

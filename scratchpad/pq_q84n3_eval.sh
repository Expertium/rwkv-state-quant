#!/usr/bin/env bash
# 84-b probe, PURE PTQ (no training): the q88N winner (norms MODELED at int4 in QAT) deployed with
# int3 norms (RWKV_PQ_NORM_BITS=3) = 88 - 4 = 84 b/card, tag q84n3p. Rationale: int4==int5 PTQ showed
...
USERS=$(tr -d '\r' < "$UF")
echo "q84n3p PTQ eval: $(echo $USERS|wc -w) users, NPROC=$NPROC"
for u in $USERS; do
  [ -f "$PRED/rust_pred_q84n3p_${u}.json" ] && continue
  ( env RAYON_NUM_THREADS=1 OMP_NUM_THREADS=1 RWKV_WEIGHTS=$W RWKV_TRACE_DIR=$REF RWKV_PRED_DIR=$PRED \
      ...
      RWKV_PQ_NORM_BITS=3 $BIN $u >/dev/null 2>&1
    cp "$PRED/rust_pred_${u}.json" "$PRED/rust_pred_q84n3p_${u}.json" ) &
  while [ "$(jobs -rp | wc -l)" -ge "$NPROC" ]; do wait -n 2>/dev/null || true; done
done; wait
echo "=== VAL SCORE 84-b PTQ probe: q84n3p (q88N weights, int3 norms). Gate +0.0025. int4 was +0.0023/+0.0006 ==="
RWKV_TRACE_DIR=$REF RWKV_PRED_DIR=$PRED python score.py "$UF" fp32 q84n3p
echo "Q84N3EVAL_DONE"

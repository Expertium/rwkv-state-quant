...
Same columns as the QAT RESULTS table (card = full stored payload: WKV PQ + token-shifts + norms;
note = 3×card; early rungs = champion recipe 1.5 ep, everything from q224e2 on = the 2.0-ep HARD CAP;
base_drift/comp_cost = qfp32−champ / deploy−qfp32). Rows are chronological.
...
| **★★ 192 b QAT (m2b4 + int2 shifts, 2.0 ep) — WIN (18:20, first GPU-eval verdict)** | **`q192_pq`** | 64 | **192** | 576 | **+0.0018/+0.0001 — both pass** | (GPU eval; base not separately run) | — | **✓ the two proven coarsenings COMPOSE. imm median +0.0013, nbad 128/400 (between 256's 103 and 224's 148), worst = familiar hard users (6787 +0.0178). Margin +0.0007 > the ±0.0002 GPU-eval tolerance ⇒ no CPU tiebreak needed** |
...
| **★★ 128 b QAT (m2b4 + m4b6 LEARNABLE shift cb, 2.0 ep) — WIN (20:35); TRUE 112 b after norm dedup** | **`q128L_pq`** | 64→48 | **128 (true 112)** | 384 (336) | **+0.0018/−0.0002 — both pass** | (GPU eval) | — | **✓ 64-centroid learnable shift cb holds; ahead still NEGATIVE. nbad 129/82, worst = familiar hard users. Margin +0.0007 > GPU-eval tolerance** |
| 96 b via m4b4 shift cb (q96L, 2.0 ep) | `q96L_pq` | 48 | 96 | 288 | +0.0026/+0.0005 — imm +0.0001 OVER | (GPU eval) | — | ✗ NEAR-MISS: 16-centroid shift cb finally bites (nbad 166). Superseded by the norm-quant route below |
| **★★★ 96 b via NORM QUANT — WIN (23:48), pure PTQ, CPU-eval verdict** | **`q128n4`** | 40 | **96** | 288 | **+0.0022/+0.0003 — both pass on the DEPLOY engine** | (PTQ on q128L weights) | norm-quant tax ≈ +0.0004/+0.0005 vs q128L | **✓ q128L + int4 log₂-domain norms (fixed corpus-derived ranges: WKV √σ [-3,0] oct, shifts [2.2,2.9] oct — layernorm pins them). int5 = IDENTICAL (+0.0022) ⇒ int4 norms already sufficient. NO retraining. Card = 12 BYTES. Engine flag `RWKV_PQ_NORM_BITS`** |
| 88 b = m2b3 WKV cb (8-cent, NEW rung 2026-07-05 03:32) + m4b6 learned shifts + int4 norms | `q88n4` | 32 | 88 | 264 | +0.0026/+0.0010 — imm +0.0001 OVER | (2.0-ep QAT, q128L recipe w/ m2b3) | norm tax +0.0005/+0.0004 (q88L_pq fp-norms = +0.0021/+0.0006 PASSES) | ✗ NEAR-MISS, same shape as q96L: the scheme survives the 8-centroid WKV cb (GPU readout +0.0022/+0.0001), but the un-modeled norm-quant PTQ tax tips it over. ⇒ NEXT LEVER: QAT-model the norm quant (close the LAST train/deploy gap) and retrain — should reclaim ~+0.0005 for THIS and all lower rungs |
| 80 b = m2b3 WKV + m4b5 (32-cent) learned shifts + modeled int4 norms (07:57) | `q80m4` | 32 | 80 | 240 | +0.0027/+0.0017 — imm +0.0002 OVER | (2.0-ep QAT, = q88N recipe w/ m4b5 shifts) | shift squeeze 48→40 b costs +0.0004 imm / +0.0011 ahead vs q88m4 | ✗ FAIL — the SHIFT wall again (cf. int2 shifts, q96L): halving the shift codebook (64→32 cents/pos) is a cliff, ahead especially. GPU readout +0.0026/+0.0005 under-predicted the ahead damage. Next attack: learnable WKV codebook (frees WKV bits to give back to shifts) or richer shift-cb learning |
| 80 b RETRY, BOTH codebooks LEARNED (10:17) | `q80w4` | 32 | 80 | 240 | +0.0027/+0.0018 — FAIL, identical to fixed-cb | (2.0-ep QAT, + `RWKV_QAT_PQ_LEARN=1` — NEW kernel centroid grads, STE-verified cosine 1.000000/1.7e-07 vs torch autograd ref) | learned WKV cb moved the GPU readout −0.0001 but the deploy verdict NOT AT ALL | ✗ **80 b is a CAPACITY wall, not an optimization gap**: with every learnable lever engaged (WKV cb + shift cb gradient co-training + modeled norms), m4b5 shifts still fail exactly like the fixed-cb run. 32 cents/pos cannot cover the shift manifold. Also: GPU−CPU offset GROWS with aggressiveness (ahead offset +0.0015 here) — deploy-engine CPU verdicts remain mandatory near the gate. Remaining sub-88 routes: int3 norms (84 b, PTQ-testable FREE on q88N), m1b5 joint WKV cb (84 b), role-asym shift cbs, conditional coding |
| **★★★ 84 b via INT3 NORMS — WIN (10:40), pure PTQ on the q88N weights, zero training** | **`q84n3p`** | 28 | **84** | 252 | **+0.0023/+0.0006 — statistically IDENTICAL to int4 norms (q88m4 +0.0023/+0.0006)** | (= q88N weights, `RWKV_PQ_NORM_BITS=3`) | int4→int3 norm cost = ZERO (±0.00002, noise) | **✓ the norm tax is RANGE-limited, NOT resolution-limited — int5==int4==int3 exactly. Card = 10.5 BYTES. Robustness = same profile as q88m4 (nbad 158/400, same worst users). int2-norm probe (80 b) launched — free descent while it holds** |
| **★★★ 80 b via INT2 NORMS — WIN (11:02), pure PTQ, zero training. The norm route cracks the rung the shift route double-failed** | **`q80n2p`** | 26 | **80** | 240 | **+0.0023/+0.0006 — unchanged from int3/int4 to the 4th decimal** | (= q88N weights, `RWKV_PQ_NORM_BITS=2`) | int3→int2 norm cost = ZERO | **✓ 4-LEVEL norms are still free (WKV steps of 1 octave!) — the PQ indices carry ~all the information; the norm scalar is nearly redundant. Card = 10 BYTES. Robustness same profile (nbad 159/400). Next free probes: int1 norms (76 b) + FIXED constant norms (72 b, 0 norm bits — tiny engine edit)** |
| **★★★ 76 b via INT1 NORMS — WIN (11:48), pure PTQ. THE NORM FLOOR IS AT 1 BIT** | **`q76n1p`** | 25 | **76** | 228 | **+0.0023/+0.0005 — still unchanged from int2/int3/int4** | (= q88N weights, `RWKV_PQ_NORM_BITS=1`) | int2→int1 norm cost = ZERO | **✓ TWO-LEVEL norms suffice. Card = 9.5 BYTES (242×). Robustness nbad 165/400 (trend-consistent, same worst users)** |
| 72 b via FIXED norms (0 stored bits) | `q72n0p` | 24 | 72 | 216 | +0.0064/+0.0036 — FAIL decisively | (= q88N weights, `RWKV_PQ_NORM_BITS=0`, engine mode added) | 1 bit → 0 bits = +0.0041/+0.0031 cliff | ✗ **the norm axis bottoms out at 1 bit**: collapsing 2 levels → 1 constant is where the magnitude information finally binds. (A QAT-modeled fixed-norm retrain could in principle absorb some, but the gap is 13× what norm-modeling recovered at 88 b — treated as a wall) |
| **★★★★ 72 b via JOINT m1b5 WKV cb — WIN (13:54), NEW CHAMPION, and BETTER quality than 76/80/84/88 b** | **`q72jv`** | 22 | **72** | 216 | **+0.0018/+0.0016 — imm beats every rung since 192 b** | (2.0-ep QAT: m1b5 joint 16-dim 32-cent cb LEARNABLE + m4b6 shifts LEARNABLE + 1-bit norms MODELED) | joint coding at 5 b/dir BEATS the m2 product form at 6 b/dir | **✓ THE JOINT-CODEBOOK DISCOVERY: one 32-entry catalog over the full 16-dim direction (tractable at this dim!) captures cross-chunk correlation the product form cannot — quality UP, bits DOWN. Card = 9 BYTES (256×), note 27 B. Robustness best since 144 b: nbad 133/400, Q4 +0.0024 all-quartiles-pass, 6951 ahead outlier shrank. GPU/CPU ahead offset again large (GPU −0.0001 vs CPU +0.0016) — CPU verdicts stay mandatory. Opens: m1b4 (68 b), m1b3 (64 b via WKV alone), rotation+m4b5 (64 b via shifts)** |
| 68 b via joint m1b4 WKV cb (16-entry) | `q68jv` | 18 | 68 | 204 | +0.0023/+0.0039 — AHEAD fails by +0.0014 | (2.0-ep QAT, = q72j recipe w/ m1b4) | m1b5→m1b4 halving destroys the FORGETTING-CURVE head specifically (imm survives) | ✗ FAIL — the joint-WKV floor is 32 entries (m1b5): 16 joint centroids can't carry the ahead information; m1b3 moot. Remaining 64-b route = rotation + m4b5 shifts (q64r launched) |
| **★★★★★ 64 b via LEARNED ROTATION + m4b5 shifts — WIN at the boundary (18:13). THE 8-BYTE CARD: one machine word** | **`q64rv`** | 22 | **64** | 192 | **+0.002492/+0.0012 — imm passes by 0.000008 (RAZOR-thin)** | (2.0-ep QAT: m1b5L WKV + m4b5L shifts + LEARNED Cayley pre-rotation + 1-bit norms) | the rotation recovered ~+0.0002 imm — exactly the m4b5 wall's deficit (+0.0027 → +0.00249) | **✓ SpinQuant→PQ works: cross-chunk correlation moved across the chunk boundary cracks the 32-entry shift wall that failed twice without it. 288×, note = 24 B. ⚠ WEAKEST robustness of the ladder: nbad 173/400, Q3/Q4 quartile means +0.0027/+0.0031 (power users pay); no individual blow-up. VERDICT: boundary win — 72 b stays the COMFORTABLE champion, 64 b is the at-the-gate extreme. ⚠⚠ SUPERSEDED by the q64bv seed test (see below): the exact recipe at seed 4321 FAILS both heads → this pass was seed luck, 64 b is NOT a reproducible win** |
| 56 b via rotation + m4b4 (16-entry) shifts (20:32) | `q56rv` | 22 | 56 | 168 | +0.0032/+0.0041 — BOTH heads fail decisively | (2.0-ep QAT, = q64r recipe w/ m4b4) | rotation ≠ capacity: it bought m4b5 its win but 16 entries/chunk is beyond saving | ✗ FAIL. q56k (same config + KD 0.5 + dead-centroid resurrection) auto-queued as the levers' first A/B — needs to claw back +0.0007/+0.0016 to pass, a big ask |
| 56 b A/B: + KD λ=0.5 + resurrection (22:57) | `q56kv` | 22 | 56 | 168 | +0.0034/+0.0034 — still FAILS, but the SPLIT is the finding | (= q56r + `RWKV_QAT_KD=0.5` + `RWKV_QAT_CB_RESURRECT=1`; 9 dead shift-centroids re-seeded) | KD vs no-KD: ahead −0.0007 (anchoring WORKS), imm +0.0003 (teacher pull trades against imm quant robustness at λ=0.5) | ✗ 56 b stays dead — but KD is a real AHEAD lever with an imm cost knob. Next: λ=0.2 KD + resurrection retrain of the 64-b boundary card (`q64k`) to widen its razor margin / fix its weak robustness |
| **★★★ 88 b RETRY with QAT-MODELED norm quant — WIN (05:50)** | **`q88m4`** | 32 | **88** | 264 | **+0.0023/+0.0006 — both pass, TRUE 88 b on the deploy engine** | (2.0-ep QAT, = q88L recipe + `RWKV_QAT_NORM_BITS=4`) | modeled-norm tax only ~+0.0002 (vs +0.0005 PTQ) | **✓ the norm-quant QAT lever WORKS: new CUDA op `rwkv7_set_norm_quant` (WKV √σ, [-3,0] oct) + `fake_pq_shift` norm quant ([2.2,2.9] oct), parity 1e-07 both paths. Card = 11 BYTES. Robustness PASS but tightest yet: imm nbad 158/400, Q4 mean +0.0027, worst = familiar identities; ahead outlier 6951 +0.0354 (growing slowly — watch). ⚠ NTFS case-collision incident: first "verdict" scored STALE q88n4 preds (tag q88N4 == q88n4 on Windows) — caught via impossible byte-identical scores + 2.5-min "eval"; tags are lowercase-unique from now on** |
| 64 b A/B: q64r + KD λ=0.2 + resurrection (2026-07-06 01:24) | `q64kv` | 22 | 64 | 192 | +0.0034/+0.0050 — BOTH heads fail; ahead 4× worse than q64rv | (= q64r + `RWKV_QAT_KD=0.2` + `RWKV_QAT_CB_RESURRECT=1`; 26 shift-centroid re-seeds across 11 events, vs q56k's 9) | KD+resurrection vs plain q64r: imm +0.0009, ahead +0.0038 — the "margin-widening" levers DAMAGED the boundary config | ✗ BACKFIRED. Degradation is UNIFORM (ahead Q1–Q4 quartile means all ≈+0.005, nbad 266/400 — global model damage, not a power-user tail). q56k's ahead benefit (KD −0.0007 at λ=0.5) did NOT transfer to 64 b; prime suspect = resurrection THRASH (26 re-seeds keep invalidating codes the learned rotation has already adapted to — the two levers fight). Lesson: don't stack resurrection on a rotation config that already converged without it; if retried, KD-only λ=0.2 A/B would isolate it. q64rv stays the 64-b record; q64a (anneal A/B, no KD/no resurrect) auto-queued next |
| 64 b A/B: q64r + soft-to-hard ANNEAL tau0 0.05, hard at 50% (2026-07-06 03:37) | `q64av` | 22 | 64 | 192 | +0.0033/+0.0029 — BOTH fail | (= q64r + `RWKV_QAT_SHIFT_ANNEAL=0.05`, clean A/B — no KD, no resurrection) | anneal vs plain q64r: imm +0.0009, ahead +0.0017 — soft selection early in training HURT; degradation uniform (imm nbad 205/400, ahead 221/400, all quartiles ≈flat) | ✗ FAIL — annealing is a dead lever here (soft-assignments early let the codebook/rotation co-adapt to averages the hard deploy never sees). **THE PATTERN: q64kv imm +0.003354, q64av imm +0.003348 — two INDEPENDENT perturbations of the q64r recipe land at the SAME imm ≈ +0.0033.** The q64rv +0.002492 imm pass (margin 0.000008) increasingly looks like a NARROW OPTIMUM, possibly batch-order luck → seed-variance rerun of the EXACT q64r config queued (q64b, `RWKV_AUGMENT_SEED` 1234→4321) to test reproducibility of the boundary win |
| **★★★★★ 56 b via the FULL LEVER STACK — WIN (2026-07-06 06:07). THE 7-BYTE CARD: the rung that failed twice falls** | **`q56sv`** | 22 | **56** | 168 | **+0.002443/+0.002243 — BOTH pass (imm margin 0.000057, ahead 0.000257)** | (2.0-ep QAT: m1b5L WKV + m4b4L 16-entry shifts + LEARNED rotation + ANNEAL tau0 0.05 hard-at-50% + `RWKV_QAT_KD=0.2` + resurrection ARMED) | vs q56rv (rot only): imm −0.0008, ahead −0.0019; vs q56kv (KD 0.5): imm −0.0010, ahead −0.0012 | **✓ THE LEVERS ARE CAPACITY-CONDITIONAL: anneal+KD hurt at 64 b (q64av/q64kv, config already at its optimum) but RESCUE the capacity-starved 16-entry shift wall. KEY MECHANISM: resurrection never fired (0 re-seeds, vs 26 thrashing at q64k) — the anneal's soft phase keeps all 16 centroids alive, so the winning combo is effectively rot+anneal+KD0.2. Robustness = q64rv-class (imm nbad 173/400, Q3/Q4 +0.0028, worst +0.0175, no blow-ups; ahead nbad 192, quartiles flat). 329×, note = 21 B. ⚠ Same thin-margin caveat as q64rv — seed-variance rerun of the exact recipe is the natural follow-up (q64b's answer applies to the family). Opens: 52 b (m1b4 WKV + the full stack — KD is exactly the ahead-lever the m1b4 wall needs). ⚠⚠ SUPERSEDED by the q56bv seed test (below): the exact recipe at seed 4321 FAILS both heads → this pass was ALSO seed luck; the 7-byte card is NOT a reproducible win** |
| 64 b SEED-VARIANCE test: exact q64r recipe, only `RWKV_AUGMENT_SEED` 1234→4321 (2026-07-06 08:19) | `q64bv` | 22 | 64 | 192 | +0.002585/+0.003026 — BOTH fail (imm +0.0001 over, ahead +0.0005 over) | (bit-identical config to q64rv; one seed change) | seed 1234→4321: imm +0.0001, ahead +0.0018 — **the seed alone moves ahead by 1.5× the entire gate margin** | ✗ **THE 64-b "WIN" WAS SEED LUCK — CONFIRMED.** Three q64r-family runs now: q64rv +0.00249/+0.0012 (pass), q64kv/q64av imm ≈+0.0033, q64bv +0.0026/+0.0030. The recipe's true mean sits AT or ABOVE the gate; q64rv drew the lucky tail. Robustness of q64bv: uniform fail (imm nbad 185/400, ahead 221/400, quartiles flat; one ahead outlier 6946 +0.057). VERDICT: 64 b via rot+m4b5 is NOT a reproducible win — q64rv's row now carries the caveat. ⚠ Direct implication for q56sv (margin 0.000057 imm): same-family thin margin, seed-variance test q56b is now MANDATORY before the 7-byte card is called real. 72-b q72jv (+0.0018/+0.0016, margin 0.0007/0.0009) remains the last rung with margin > seed noise |
| 52 b via BOTH 16-entry walls + full lever stack (2026-07-06 10:45) | `q52sv` | 18 | 52 | 156 | +0.003586/+0.003039 — BOTH fail decisively | (2.0-ep QAT: m1b4L joint WKV 16 b + m4b4L shifts 32 b + rotation + anneal tau0 0.05 + KD 0.2 + resurrection + 1-bit norms) | vs q56sv (same stack, m1b5 WKV): imm +0.0011, ahead +0.0008 — the m1b4 halving costs BOTH heads even with every lever engaged | ✗ **the m1b4 WKV wall STANDS: the lever stack rescued the 16-entry SHIFT wall (56 b) but cannot rescue the 16-entry JOINT-WKV wall on top of it.** Uniform failure (imm nbad 222/400, ahead 228/400, all quartiles over gate; familiar worst users, no blow-ups). 32 joint-WKV centroids (m1b5) is the confirmed floor with-and-without levers → 56 b is the END of this ladder's bit descent (pending its q56b seed test); no cheaper WKV encoding is on the table |
| 56 b SEED-VARIANCE test: exact q56s recipe, only `RWKV_AUGMENT_SEED` 1234→4321 (2026-07-06 13:12) | `q56bv` | 22 | 56 | 168 | +0.002924/+0.003390 — BOTH fail (imm +0.0004 over, ahead +0.0009 over) | (bit-identical config to q56sv; one seed change) | seed 1234→4321: imm +0.0005, ahead +0.0011 — same swing magnitude as the 64-b family | ✗ **THE 56-b "WIN" WAS ALSO SEED LUCK.** The q56sv +0.002443/+0.002243 pass does not reproduce; the recipe's true mean sits above the gate on both heads. Uniform fail (imm nbad 193/400, ahead 235/400, quartiles flat, familiar worst users). CONSOLIDATED SEED-NOISE PICTURE: two exact-recipe seed pairs measured — imm swing +0.0001/+0.0005, ahead swing +0.0018/+0.0011 → **any at-the-gate pass with margin < ~0.001 imm / ~0.002 ahead is unresolvable by a single run.** Consequence: 64 b and 56 b are both DEAD as wins; **q72jv (margin 0.0007/0.0009) is the presumptive champion but its ahead margin is INSIDE the observed swing → q72b (exact q72j recipe, seed 4321) launched to test the champion itself.** If q72b holds, 72 b = 9-byte card stands as the honest floor; if not, the ladder's last certain rung is 76 b (q76n1p, margin 0.0002 imm... also thin) / 88 b (q88m4, +0.0023/+0.0006) — the whole sub-100-b tail may need multi-seed verdicts |
| **★★★★★ 72 b SEED-VARIANCE test — THE CHAMPION REPRODUCES (2026-07-06 15:15). THE LADDER IS RESOLVED: 72 b = the honest floor, 9-BYTE CARD** | **`q72bv`** | 22 | **72** | 216 | **+0.001931/+0.000768 — BOTH pass AGAIN (margins 0.0006/0.0017)** | (bit-identical config to q72jv; one seed change, 1234→4321) | seed swing at 72 b: imm +0.0001, ahead −0.0008 — the recipe's mean sits safely UNDER the gate, unlike the 64/56-b families whose means sat ON it | **✓ 2/2 seeds pass with margin: q72jv +0.0018/+0.0016, q72bv +0.0019/+0.0008. Robustness best of the endgame (imm nbad 142/400, ahead 137, ALL quartiles pass both heads, familiar worst users, no blow-ups). VERDICT: the JOINT m1b5 recipe (m1b5L 32-entry joint WKV cb 20 b + m4b6L 64-entry shift cb 48 b + 1-bit norms 4 b, plain 2.0-ep QAT, no rotation/anneal/KD) is the CONFIRMED champion at 72 b/layer = 9 B/card, 27 B/note, 256×. Below it every rung's true mean is at/above the gate (56/64 b seed luck, 52 b outright). END OF THE BIT DESCENT** |
...
| **★★★★★ 72 b SEED-VARIANCE test of q72c — THE NEW CHAMPION REPRODUCES (2026-07-07 00:49)** | **`q72dv`** | 23 | **72** | 216 | **+0.001650/+0.000509 — BOTH pass AGAIN (margins 0.00085/0.00199)** | (bit-identical config to q72cv; one seed change, 1234→4321; run was killed at ~73% by the 2026-07-06 PC restart and relaunched from scratch) | seed swing at 72 b m2b12: imm +0.00036, ahead +0.00030 — half the swing of the q72j family, an order below the 64/56-b families | **✓ 2/2 seeds pass with wide margin: q72cv +0.001295/+0.000212, q72dv +0.001650/+0.000509 — the m2b12 big-catalog champion is SEED-ROBUST on both heads. Robustness reproduces too (imm nbad 119/400 vs cv's 108, ahead 124 vs 106, quartiles flat, worst ≈+0.02, no blow-ups). **q72c is the confirmed 72-b champion**; 72 b locked as the deploy target (Andrew 2026-07-06) — descent closed, quality campaign continues: q72u (joint-uv WKV catalog) auto-queued next, then q72f (shift rotation on m2b12)** |
| **★★★★ 72 b via JOINT-UV WKV catalog — edges the champion on EVERY axis, best imm robustness ever (2026-07-07 04:29)** | **`q72uv`** | 23 | **72** | 216 | **+0.001142/+0.000214 — both pass; vs q72cv: imm −0.00015, ahead tie (+0.000002)** | (exact q72c recipe, WKV cb swapped m1b5 → JOINT-UV b10: per head ONE 10-bit code into a 1024-entry catalog of concat(u_unit,v_unit) 32-dim entries = SAME 20 WKV b/card; k-means from 111,480 paired dirs; learnable; kernel joint branch parity 32/32 at ≤2.6e-07 both norm modes) | joint vs product AT IDENTICAL INDEX SPACE (32×32 product = 1024 combos vs 1024 free entries): the entire gain = non-product u/v correlation → measured ≈ +0.0002 imm — the product form was already near-optimal | **✓ WEAKLY DOMINANT: never worse on any axis — imm mean −0.00015, ahead tie, imm nbad 96/400 (BEST EVER, cv was 108), ahead nbad 106 (tie), Q3-ahead NEGATIVE, no blow-ups. But the margin is inside the family seed swing (±0.0004) → champion title needs the seed pair: **q72w (seed 4321) queued behind q72f**. Science note: u/v correlation exists but is ~exhausted by the product form at this operating point — joint coding buys robustness more than mean. Cost: −17% train throughput (49 vs 59 steps/min, the 1024-entry in-kernel search; tiling fix on the speed list)** |
| 72 b: LEARNED SHIFT ROTATION on the m2b12 champion (2026-07-07 08:03) | `q72fv` | 23 | 72 | 216 | +0.001902/+0.001217 — passes the gate but LOSES to q72cv on BOTH heads (imm +0.0006, ahead +0.0010) | (exact q72c recipe + `RWKV_QAT_SHIFT_ROT=1`, learned 2×32×32 Cayley on the shift vectors before the m=2 chunk split; deploy `RWKV_SHIFT_ROT` verified in the eval env) | rotation vs no-rotation at 4096-entry catalogs: NEGATIVE — the big catalog already captures what the rotation would move; the extra 2048 rotation params just churn the selection basis mid-training | ✗ **ROTATION IS NOW FULLY RESOLVED: (a) m=1 WKV — provably absorbed by the learnable catalog (see analysis note); (b) m=2 BIG-catalog shifts — empirically negative (this row; robustness also worse, nbad 139/143 vs uv's 96/106, Q3/Q4 imm near the gate); (c) its only wins were capacity-starved SMALL catalogs (m4b5@64 b, m4b4@56 b — both rungs later died as seed luck). The lever is closed. 72-b hierarchy now: q72uv +0.00114/+0.00021 (provisional champ, seed test q72w running) > q72cv +0.00130/+0.00021 > q72fv > q72jv/q72bv** |
| **★★★★★ 72 b JOINT-UV SEED test — NEW CONFIRMED CHAMPION (2026-07-07 11:39): joint-uv wins the seed-paired duel on BOTH heads** | **`q72wv`** | 23 | **72** | 216 | **+0.001150/+0.000403 — both pass; imm seed swing vs q72uv = +0.000008 (essentially ZERO)** | (bit-identical config to q72uv; one seed change, 1234→4321) | SEED-PAIRED DUEL, joint-uv vs m2b12-product: imm 0.001142/0.001150 vs 0.001295/0.001650 (better at BOTH seeds), ahead 0.000214/0.000403 vs 0.000212/0.000509 (tie + better); pair means imm +0.00115 vs +0.00147, ahead +0.00031 vs +0.00036 | **✓ THE 72-b CHAMPION IS NOW q72u (JOINT-UV): m2b12L shifts 48 b + joint-uv b10 WKV catalog 20 b + 1-bit norms 4 b = 72 b, 9-byte card, 256×. Robustness reproduces (imm nbad 98 vs uv's 96 — best-ever class at both seeds; Q3-ahead negative both seeds). The imm head is nearly seed-noise-FREE under joint coding (+0.000008 swing vs product's +0.00036) — joint u/v coding stabilizes exactly the head the deploy cares most about. Deploy artifacts: reference/qat_pq_q72u.safetensors + pq_cb_wkv_q72u.txt (joint) + pq_cb_shift_q72u.txt. Quality campaign at 72 b: 4 levers resolved (rotation closed, EMA/soup dead, joint-uv = champion) → remaining thread = SPEED (round-4 compile A/B fired 11:39, CUDA graphs, joint-search tiling)** |

**★ QAT SPEED ROUND 4 SETTLED (2026-07-07 11:55): `RWKV_QAT_COMPILE=student` + the 4 flags = soft-phase
913.7 ms/step vs round-3's 1157.8 (−21%).** The round-3 compiled-teacher regression is gone (teacher_fwd
343→99.5 ms eager); combined fwd 607→546 ms, backward 505→338 ms (compiled-student backward is the big
winner). Production adoption: the flag set (`ROT_CACHE`+`FAST_EMB`+`EMA_FOREACH`+`NO_MEMFILL`+
`COMPILE=student`) is sanctioned for FUTURE training families (Andrew OK'd trajectory perturbation;
never flip flags inside a comparison family). Remaining speed levers: CUDA graphs (round-3 profile still
...
checkpoint → wrote `DONE_EXIT_1` → the verdict chain aborted (correctly) BUT its own DONE_EXIT_1 in
`pq_q72f.log` triggered q72w's training waiter → **q72w trained alongside q72f for ~8 min** (killed;
deterministic kernels → q72f's trajectory unaffected, only slower). FIXES: wait windows 960→2400 ticks
AND expiry now `goto :eof` (die silently; the chain's own 25-h timeout is the reporter) — a stage that
times out must NOT write the same marker a success writes. q72w wrapper was killed FIRST when cleaning
up so no false `DONE_EXIT` got echoed; stale logs deleted; all stages relaunched 06:32.
...
one job at a time.** Training OMP4+fetch4; GPU eval fetch 5; RAM is first-class (three concurrent fetcher
pools + queues held ~30 GB). Chains self-serialize via log markers (train → GPU eval → CPU verdict, each
stage polls the previous stage's DONE_EXIT; monitors grep `DONE_EXIT_[0-9]`, never bare DONE_EXIT).

**★ 2026-07-05 SESSION SUMMARY — the descent 96 → 64 b and the lever inventory.**
CHAMPIONS: **72 b `q72jv` +0.0018/+0.0016 = the COMFORTABLE champion** (9 bytes, 256×, robustness best
since 144 b) · **64 b `q64rv` +0.002492/+0.0012 = the boundary extreme** (8-byte machine word, 288×,
weakest robustness of the ladder). FLOORS MAPPED: **norms = 1 bit** (int5..int1 all identical; 0-bit
fixed FAILS +0.0064) · **shift catalogs = 32 entries/chunk WITH the learned rotation, 64 without**
(16 entries fails even with the rotation) · **joint WKV catalog = 32 entries** (16 kills the AHEAD
head specifically). LEVERS PROVEN THIS SESSION: (1) QAT-modeled norm quant `RWKV_QAT_NORM_BITS`
(PTQ tax +0.0005 → ~+0.0002; turned the 88-b near-miss into a pass); (2) LEARNABLE WKV codebook
`RWKV_QAT_PQ_LEARN` (kernel centroid grads, STE-verified cosine 1.000000 — helps GPU readout, can NOT
fix capacity walls); (3) JOINT 16-dim WKV codebook (m1b5 5 b/dir BEATS the m2 product form at 6 b/dir —
quality up, bits down); (4) LEARNED Cayley shift pre-rotation `RWKV_QAT_SHIFT_ROT` (SpinQuant→PQ;
cracked the m4b5 wall, worth ~+0.0002 imm; moves correlation, not capacity). NEW LEVERS BUILT, FIRST
A/B RUNNING (q56k): (5) KD from the un-quantized fp32 champion `RWKV_QAT_KD` (soft-label CE+BCE, both
heads, teacher = second model instance with QAT hooks stripped); (6) dead-centroid resurrection
`RWKV_QAT_CB_RESURRECT` (grad-norm EMA, re-seed at the busiest centroid — 16-entry shift catalogs DO
develop dead entries: 7 re-seeds by mid-run). APPROVED, NOT YET BUILT: soft-to-hard selection annealing
(must end fully hard — Andrew's condition); WKV sandwich rotation (rotate the state before rank-1).
METHOD NOTES: NTFS case-collision → eval tags lowercase-unique · you cannot finite-difference-check STE
gradients (use an autograd port of the same semantics) · GPU−CPU ahead-offset GROWS with scheme
aggressiveness (up to +0.0018) → deploy-engine CPU verdicts mandatory near the gate.
